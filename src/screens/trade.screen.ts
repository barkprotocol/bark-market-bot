  ...
  if (quoteResult) {
    ...
    if (status) {
      ...
      const pnlData = {
        ...
        isBuy: false,
        referralLink: `https://t.me/${BarkBotID}?start=${referrerCode}`,
      };
...
export const setSlippageHandler = async (
  ...
  try {
    ...
    if (percent < 0 || percent > 100) {
      await bot.sendMessage(chat_id, "Invalid slippage percentage. Please enter a value between 0 and 100.");
      return;
    }
    ...
    if (!msglog) {
      console.error(`Message log not found for username: ${username}`);
      await bot.sendMessage(chat_id, "Slippage setting details not found. Please try again.");
      return;
    }
    const { mint, parent_msgid, msg_id } = msglog;

    if (!mint) {
      console.error("Mint address not found in message log");
      await bot.sendMessage(chat_id, "Token information not found. Please try again.");
      return;
    }
    ...
    if (!user) {
      ...
      await bot.sendMessage(chat_id, "User not found. Please try again.");
      return;
    }
    ...
    await bot.sendMessage(chat_id, `Slippage has been set to ${percent}%`);
  } catch (error) {
    console.error("Error in setSlippageHandler:", error);
    await bot.sendMessage(msg.chat.id, "An error occurred while setting the slippage. Please try again later.");
  }
};
