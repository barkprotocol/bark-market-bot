...
import { BarkBotVersion, MAX_WALLET, private_connection } from "../config";
...
export const switchBurnOptsHandler = async (
  ...
  try {
    ...
    if (!user.burn_fee) {
      const caption =
        `Burn: On 🔥\n\n` +
        `BarkBOT's burn functionality operates seamlessly through its fee system, where a portion of tokens bought and sold is systematically burned. This process does not affect users' own tokens but only those acquired through the fee mechanism, ensuring the safety of your trades.`;
