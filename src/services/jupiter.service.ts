...
function fromWeiToValue(wei: string | number, decimals: number): number {
...
const config = {
  basePath: "https://barkbot.fly.dev"
}
...
export class JupiterService {
  ...
  async swapToken(
    ...
    try {
      ...
      console.error("Error in swapToken:", e);
      return null;
    }
  };
  ...
  async getQuote(
    ...
    try {
      ...
      console.error("Error in getQuote:", e);
      return null;
    }
  };
  ...
  async transferSOL(fundAmount: number, decimals: number, toPubkey: string, pk: string, lamports: number = 5000, units: number = 20000): Promise<string | null> {
  ...
  async transferSPL(mint: string, fundAmount: number, decimals: number, toPubkey: string, pk: string, isToken2022: boolean): Promise<string | null> {
    if (fundAmount <= 0) return null;
    const wallet = Keypair.fromSecretKey(bs58.decode(pk));
